...
var LocationModel = Backbone.Model.extend({
    ...
    parse: function(response) {
        return response.event_time = response.event_time_human, response.url = response.event_url_stub, response
    }
    ...
    queryParams: {
        ...
        longitude: "",
        limit: "8",
        image_size: "mobile-retina"
    },
    model: Event,
    url: function() {
        var params = $.param(this.queryParams);
        return "/events/?" + params
    }
});
!function($, window) {
    var $container = $(".home-container");
    window.locationModel = new LocationModel({
        q: $container.data("address"),
        latitude: $container.data("latitude"),
        longitude: $container.data("longitude")
    })
}(jQuery, window), $(document).ready(function() {
    var ChangeLocationView = Backbone.View.extend({
        el: ".change-location-container",
        events: {
            "click .change-loc-btn": "toggleBody",
            "keyup .change-loc-body input": "autocomplete",
            "click .change-loc-results li": "changeLocation"
        },
        initialize: function() {
            this.model.bind("change:results", this.render, this), this.template = _.template(this.$el.find("#change-loc-template").html()), this.$locationText = this.$el.find(".change-loc-text"), this.$placesInput = this.$el.find(".change-loc-body"), this.$placesResults = this.$el.find(".change-loc-results"), this.isVisible=!1, this.populateLocationText()
        },
        toggleBody: function() {
            this.isVisible ? (rukBrowserInfo.isIE9 ? this.$placesInput.css("opacity", 0) : this.$placesInput.snabbt({
                opacity: 0,
                fromOpacity: 1,
                duration: 200,
                easing: "ease"
            }), this.model.reset()) : rukBrowserInfo.isIE9 ? this.$placesInput.css("opacity", 1) : this.$placesInput.snabbt({
                opacity: 1,
                fromOpacity: 0,
                duration: 200,
                easing: "ease"
        ...
        autocomplete: function(evt) {
            var term = $(evt.currentTarget).val();
            this.model.set({
                q: term
            }), this.model.placesSearch()
        },
        populateLocationText: function() {
            var location_parts = window.locationModel.get("q").split(","), location = location_parts[0] + "," + location_parts[1];
            this.$locationText.html(_.template("<%= loc %>")({
                loc: location
            }))
        },
        changeLocation: function(evt) {
            var changeTo = $(evt.currentTarget).text();
            window.locationModel.set({
                q: changeTo
            }), this.toggleBody(), this.populateLocationText()
        },
        render: function() {
            this.$placesResults.html(this.template(this.model.attributes))
        }
    });
    new ChangeLocationView({
        model: new PlacesModel
    })
}), $(document).ready(function() {
    var EventsForLocationView = Backbone.View.extend({
        el: ".event-list-box.nearby .nearby-events-container",
        template: _.template($("#event-list-headshot-tpl").html()),
        initialize: function() {
            window.locationModel.bind("change:latitude", this.loadEvents, this), this.collection.bind("sync", this.render, this), this.$loader = $(".spinner")
        },
        loadEvents: function() {
            this.collection.queryParams.latitude = window.locationModel.get("latitude"), this.collection.queryParams.longitude = window.locationModel.get("longitude"), this.collection.fetch(), this.$el.html(this.$loader), this.$loader.show()
        },
        render: function() {
            this.$el.parent().find(".nearby-events-header-loc").text(window.locationModel.get("q")), this.$el.html(this.template({
                events: this.collection.models
            }))
        }
    });
    new EventsForLocationView({
        collection: new EventCollection
    });
    var onScroll = function() {
        var isOpaque=!1, $nav = $(".nav-bar-container"), $searchContainer = $(".mobile-nav-bar .mobile.search-container"), wasScrolled=!1;
        return setInterval(function() {
            if (wasScrolled) {
                var TOP_THRESHOLD = 300, scrollTop = $(window).scrollTop();
                scrollTop > TOP_THRESHOLD&&!isOpaque ? ($nav.addClass("nav-on-scroll"), isOpaque=!0, isMobile() && $searchContainer.fadeIn()) : isOpaque && TOP_THRESHOLD > scrollTop && ($nav.removeClass("nav-on-scroll"), isOpaque=!1, isMobile() && $searchContainer.fadeOut()), wasScrolled=!1
            }
        }, 100), function() {
            wasScrolled=!0
        }
    };
    $(window).on("scroll", onScroll()), rukBrowserInfo.isIE && $(".top-scroll-container").addClass("top-scroll-container-ie"), rukBrowserInfo.isIE9 && ($(".slogan-header").css("opacity", 1), $(".search-container").css("opacity", 1)), isMobile() || (skrollr.init({
        forceHeight: !1
    }), SearchBoxAutocomplete.create(".home-search-container"))
});
